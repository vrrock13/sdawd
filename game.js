...
const questionTemplates = [
  { id: "id", label: "신분증 확인" },
  { id: "permit", label: "출입 요청서 확인" },
  { id: "phone", label: "거주지 전화" },
  { id: "accessory", label: "액세서리 대조" },
  { id: "reason", label: "방문 목적 추궁" },
  { id: "history", label: "이전 방문 기록" }
];
...
const state = {
  ...
  stats: { approved: 0, rejected: 0, emergency: 0, impostersCaught: 0, innocentsLost: 0 },
  lostBooks: new Set()
};
...
const cMap = {"신분증 확인":"id","출입요청서 확인":"permit","거주지 전화":"phone","액세서리 대조":"accessory","방문 목적 추궁":"reason","이전 방문 기록":"history"};
...
function buildVisitor(){
  const resident=pick(residentDirectory); const isImposter=Math.random()<0.36;
  const shownName=isImposter && Math.random()<0.72 ? pick(residentDirectory).name : resident.name;
  const shownApt=isImposter && Math.random()<0.62 ? pick(residentDirectory).apt : resident.apt;
  const shownAccessory=isImposter && Math.random()<0.7 ? pick(accessories) : resident.accessory;
  const idDoc={name:shownName,apt:shownApt,id:resident.id,expiry:1955+Math.floor(Math.random()*3)};
  const permitDoc={apt:Math.random()<0.12 ? pick(residentDirectory).apt : shownApt, seal:Math.random()>(isImposter?0.45:0.05)};
  const flags=[]; if(idDoc.name!==resident.name) flags.push('신분증 이름 불일치'); if(idDoc.apt!==resident.apt) flags.push('신분증 거주지 불일치');
  if(!permitDoc.seal) flags.push('요청서 DDD 봉인 없음'); if(permitDoc.apt!==resident.apt) flags.push('요청서 거주지 불일치'); if(shownAccessory!==resident.accessory) flags.push('액세서리 불일치');
  return {resident,isImposter,shownName,shownAccessory,idDoc,permitDoc,flags,suspicious:(isImposter?28:7)+flags.length*6+Math.floor(Math.random()*10),trait:`${pick(['경계','초조','침착','과장된 친절','피로'])}한 태도. 직업 주장: ${pick(jobs)}.`};
}

function renderMemoPanel(){const checks=['신분증 확인','출입요청서 확인','거주지 전화','액세서리 대조','방문 목적 추궁','이전 방문 기록']; el.memoPanel.innerHTML=checks.map(c=>`<label><input type="checkbox" disabled ${state.askedQuestions.has(cMap[c])?'checked':''}/> ${c}</label>`).join('');}
...
function askQuestion(type){
  ...
  const ans={
    id:`신분증: ${v.idDoc.name} / ${v.idDoc.apt} / 만료 ${v.idDoc.expiry}`,
    permit:`출입요청서: ${v.permitDoc.apt} / 봉인 ${v.permitDoc.seal?'정상':'누락'}`,
    phone:`거주지 전화: ${v.isImposter?'세대원이 이미 있다고 응답':'본인 귀가 예정 응답'}`,
    accessory:`액세서리: 현재 '${v.shownAccessory}' / 주민표 '${v.resident.accessory}'`,
    reason:`진술: ${state.currentRequest.text}`,
    history:`기록: ${v.isImposter?'동일 신분 의심 제보 존재':'최근 문제 없는 출입 기록'}`
  };
  v.suspicious = Math.max(4, v.suspicious + ((v.flags.length && ['id','permit','accessory','phone'].includes(type))?3:-2));
  el.trustHint.textContent=`신뢰도 단서: 거짓말 확률 추정 ${v.suspicious}%`;
  el.docStatus.textContent=`문서 상태: ${v.flags.length?v.flags.join(', '):'이상 없음'}`;
  state.radio.push(`심문: ${ans[type]}`); renderLogs(); renderQuestionPanel();
}
...
function renderLogs(){el.resultLog.innerHTML=[...state.logs].slice(-7).reverse().map(v=>`<li>${v}</li>`).join(''); el.radioLog.innerHTML=[...state.radio].slice(-8).reverse().map(v=>`<li>${v}</li>`).join(''); el.dataBox.textContent=`승인 ${state.stats.approved} | 거부 ${state.stats.rejected} | 비상 ${state.stats.emergency} | 도플처리 ${state.stats.impostersCaught} | 오판 ${state.stats.innocentsLost}`;}
function updateHud(){el.day.textContent=`Day ${state.day}`; const temp=Math.max(-10,8-Math.floor((20-state.fuel)/3)); el.tempFuel.textContent=`🌡 온도 ${temp}℃  ⛽ 연료 ${state.fuel}`; el.foodHunger.textContent=`🍞 식량 ${state.food}  🧍 허기 ${state.hunger}`; el.threat.textContent=`⚠ 위협도 ${state.threat} (${threatLabel(state.threat)})`;}

function nextVisitor(){state.currentVisitor=buildVisitor(); const r=pick(requests); state.currentRequest={id:r.id,text:pick(requestTextPool[r.id])}; state.selectedBookId=null; state.askedQuestions.clear(); state.questionPanelOpen=false; el.visitorName.textContent=`${state.currentVisitor.shownName} (${state.currentVisitor.resident.apt})`; el.visitorTrait.textContent=state.currentVisitor.trait; el.requestText.textContent=state.currentRequest.text; el.trustHint.textContent=`신뢰도 단서: 거짓말 확률 추정 ${state.currentVisitor.suspicious}%`; el.docStatus.textContent='문서 확인 전'; el.bookDetail.textContent='도서를 선택하세요.'; renderBooks(); renderQuestionPanel(); renderMemoPanel();}
...
function approveVisitor(){if(!state.selectedBookId){alert('먼저 도서를 선택하세요.');return;} const v=state.currentVisitor; state.stats.approved+=1; if(v.isImposter && v.flags.length>=2){state.threat+=3; state.logs.push(`Day ${state.day}: 도플갱어를 승인했습니다. 내부 침투 발생.`);} resolveBookDelivery(); flash(); advanceDay();}
function rejectVisitor(){const v=state.currentVisitor; state.stats.rejected+=1; if(!v.isImposter){state.stats.innocentsLost+=1; state.threat+=2; state.food=Math.max(0,state.food-1); state.logs.push(`Day ${state.day}: 정상 주민을 돌려보내 불만이 폭증했습니다.`);} else {state.stats.impostersCaught+=1; state.threat=Math.max(0,state.threat-1); state.logs.push(`Day ${state.day}: 위장 방문자를 거부했습니다.`);} state.radio.push('무전: 출입 거부 처리 완료.'); flash(); advanceDay();}
function emergencyCall(){const v=state.currentVisitor; state.stats.emergency+=1; if(v.isImposter){state.stats.impostersCaught+=1; state.threat=Math.max(0,state.threat-2); state.logs.push(`Day ${state.day}: D.D.D. 호출 성공. 위장체 제거.`);} else {state.stats.innocentsLost+=1; state.threat+=3; state.food=Math.max(0,state.food-1); state.logs.push(`Day ${state.day}: 오판으로 주민이 사망했습니다.`);} state.radio.push('긴급 무전: D.D.D. 출동. 호출은 취소 불가.'); notify('비상 호출 전송됨'); flash(); advanceDay();}
function toggleQuestionPanel(){state.questionPanelOpen=!state.questionPanelOpen; state.radio.push(state.questionPanelOpen?'심문 도구를 펼쳤습니다.':'심문 도구를 닫았습니다.'); renderQuestionPanel(); renderLogs();}
