...
function renderLogs(){el.resultLog.innerHTML=[...state.logs].slice(-7).reverse().map(v=>`<li>${v}</li>`).join(''); el.radioLog.innerHTML=[...state.radio].slice(-8).reverse().map(v=>`<li>${v}</li>`).join(''); el.dataBox.textContent=`대출승인 ${state.stats.approved} | 반려 ${state.stats.rejected} | 격리 ${state.stats.emergency} | 위협차단 ${state.stats.threatsStopped} | 오판 ${state.stats.trustLosses}`;}
